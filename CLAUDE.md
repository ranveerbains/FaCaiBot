...
│   ├── confidence.rs              # Confidence scoring (magnitude-relative spike + depth + time), round_to_tick()
...
- **Zero-alloc tier guard**: If `tier_pct == 0` for any tier, the signal is rejected (`Leg1RejectReason::Other`) before allocation. Currently LOW tier is disabled via `low_alloc_pct = 0` in config.toml — its 1-tick margin is structurally insufficient to survive repricing. Re-enable by setting `low_alloc_pct` to a non-zero value
